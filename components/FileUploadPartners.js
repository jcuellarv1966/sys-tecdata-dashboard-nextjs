...
export default function FileUploadPartners({ values, setValues, loadingUpload }) {
    const fileUploadAndResize = async (e) => {
        if (values.image !== "") {
            const res = await axios.post("/api/admin/partners/imagesdeleted", { deletedImage: values.image });
                    ...
                    async () => {
                        ...
                        try {
                            const res = await axios.post(
                                "/api/admin/partners/imagesupload",
                                form,
                                config
                            );
    ...
    const handleImageRemove = async (image) => {
        try {
            // eslint-disable-next-line no-unused-labels
            loadingUpload: true;
            const res = await axios.post("/api/admin/partners/imagesdeleted", {
                deletedImage: image,
            });
            console.log(res.data);
            setValues({ ...values, image: "" });
                    ...
                    <>
                        <div className="relative">
                            <div className="flex items-center justify-center w-72 h-120 mx-2 overflow-hidden rounded-2xl">
                                <Image
                                    src={`/uploads/partners/${values.image}`}
