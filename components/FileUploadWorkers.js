...
export default function FileUploadWorkers({ values, setValues, loadingUpload }) {
    const fileUploadAndResize = async (e) => {
        if (values.image !== "") {
            const res = await axios.post("/api/admin/workers/imagesdeleted", { deletedImage: values.image });
            ...
            for (let i = 0; i < files.length; i++) {
                Resizer.imageFileResizer(
                    ...
                    async () => {
                        ...
                        form.append("workerImage", e.target.files[i]);
                        try {
                            const res = await axios.post(
                                "/api/admin/workers/imagesupload",
                                form,
                                config
                            );
    ...
    const handleImageRemove = async (image) => {
        try {
            // eslint-disable-next-line no-unused-labels
            loadingUpload: true;
            const res = await axios.post("/api/admin/workers/imagesdeleted", {
                deletedImage: image,
            });
            console.log(res.data);
            setValues({ ...values, image: "" });
                    ...
                    <>
                        <div className="relative">
                            <div className="flex items-center justify-center w-72 h-120 mx-2 overflow-hidden rounded-2xl">
                                <Image
                                    src={`/uploads/workers/${values.image}`}
...
FileUploadWorkers.auth = { adminOnly: true };