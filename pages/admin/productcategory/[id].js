...
export default function AdminProductCategoryEditScreen() {
    ...
    const fetchData = async () => {
      try {
        ...
        const { data } = await axios.get(
          `/api/admin/productscategories/${categoryId}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`/api/admin/productscategories/${categoryId}`, {
        values,
      });
  ...
  return (
    <Layout title={`Edit Product Category ${categoryId}`}>
        ...
        <div className="grid-2">
          {loading ? (
            ...
            <form className="mx-auto max-w-screen-lg" onSubmit={handleSubmit}>
              <h1 className="mb-4 text-xl">{`Edit Product Category ${categoryId}`}</h1>
              <div className="mb-4">
...
AdminProductCategoryEditScreen.auth = { adminOnly: true };
