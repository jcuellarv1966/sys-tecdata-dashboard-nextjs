...
export default function AdminProductsCategoriesScreen() {
  const [
  ...
  const deleteHandler = async (categoryId) => {
    ...
    try {
      ...
      await axios.delete(`/api/admin/productscategories/${categoryId}`);
  ...
  return (
    <Layout title="Admin Products Categories">
      <div className="grid md:grid-cols-8 md:gap-1 mt-4">
        <div>
          <SideNavbar />
        </div>
        <div className="overflow-x-auto md:col-span-7 px-12">
          <div className="flex justify-between mb-6">
            <h1 className="mb-2 text-xl">Products Categories</h1>
            {loadingDelete && <div>Deleting item...</div>}
            <Link href="/admin/productcategory/productcategorycreate">
                    ...
                    {categories.map((category) => (
                      <tr key={category._id} className="border-b">
                        ...
                        <td className=" p-1 w-14">
                          <Link href={`/admin/productcategory/${category._id}`}>
                            <a>
                              <FontAwesomeIcon
                                icon={faPencilAlt}
...
AdminProductsCategoriesScreen.auth = { adminOnly: true };
