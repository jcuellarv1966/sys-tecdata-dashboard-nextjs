...
function reducer(state, action) {
    switch (action.type) {
        ...
        case "FETCH_SUCCESS":
            return { ...state, loading: false, providers: action.payload, error: "" };
        case "FETCH_FAIL":
...
export default function AdminClientsScreen(props) {
    ...
    const submitHandler = (e) => {
        ...
        if (querySearch === '') {
            ...
            router.push(`/admin/providers?query=${querySearch}`);
        }
    };
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                dispatch({ type: "FETCH_SUCCESS", payload: props.providers });
            } catch (err) {
    ...
    const deleteHandler = async (providerId, image) => {
        ...
        try {
            ...
            if (image && image !== "") {
                await axios.post("/api/admin/providers/imagesdeleted", {
                    deletedImage: image,
                });
            }
            ...
            <div className="grid md:grid-cols-8 md:gap-1 mt-4">
                ...
                <div className="overflow-x-auto md:col-span-7 ml-12">
                    <div className="flex justify-between">
                        <h1 className="mb-4 text-xl">Providers</h1>
                        {loadingDelete && <div>Deleting item...</div>}
                        <Link href="/admin/provider/providercreate">
                        ...
                        <div className="overflow-x-hidden">
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full">
                                    ...
                                    {
                                        <tbody>
                                            {providers.filter(searched(keyword)).map((provider) => (
                                                <tr key={provider._id} className="border-b">
                                                    ...
                                                    <td className=" p-1 text-right">
                                                        <NumberFormat
                                                            value={Number.parseFloat(provider.credit).toFixed(2)}
                                                            displayType={"text"}
                                                    ...
                                                    <td className=" p-1 ">
                                                        <Link href={`/admin/provider/${provider._id}`}>
                                                            <a>
                                                                <FontAwesomeIcon
                                                                    icon={faPencilAlt}
                                                    ...
                                                    <td className=" p-1 ">
                                                        <a onClick={() => deleteHandler(provider._id, provider.image)}>
                                                            <FontAwesomeIcon
                                                                icon={faTrashAlt}
...
export async function getServerSideProps({ query }) {
    ...
    const clientDocs = await Provider.find({
        ...queryFilter,
    })
        .sort('razSocial')
    ...
    const countClients = await Provider.countDocuments({
        ...queryFilter,
    });

    await db.disconnect();

    const providers = clientDocs.map(db.convertDocToObjClient);
