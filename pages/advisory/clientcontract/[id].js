...
export default function AdminScreenClientContractEdit() {
    ...
    const loadContract = async () => {
        ...
        const { data } = await axios.get(`/api/advisory/clientscontracts/${contractId}`);
        ...
        for (let i = 0; i < contractItems.length; i++) {
            const { data } = await axios.get(`/api/admin/products/${contractItems[i]._id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(`/api/advisory/clientscontracts/${contractId}`, { values });
            dispatching({ type: 'CART_CLEAR_ITEMS' });
            Cookies.set(
                'cart',
                JSON.stringify({
    ...
    return (
        <Layout title={`Edit Contract of Client`}>
            <div className="grid md:grid-cols-8 md:gap-1 mt-3">
                <div>
                    <SideNavbarAdvisory />
                </div>
                <div className="overflow-x-auto md:col-span-7 ml-12">
                    {loading ? (
                        ...
                        <form className="mx-auto max-w-screen-lg" onSubmit={handleSubmit}>
                            <h1 className="mb-1 text-xl">{`Edit Client Contract ${contractId}`}</h1>
...
AdminScreenClientContractEdit.auth = { adminOnly: true };